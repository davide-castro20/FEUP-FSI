...
The week #4 challenge was based on an E-commerce WordPress website that was vulnerable by nature. The vulnerabilities encountered were related to the WordPress Plugins used in it. 
...
- Additionaly, we also found out the website was using three Wordpress plugins, WooCommerce 5.7.1, WooCommerce Booster 5.4.3, and Storefront.
...
- Knowing that, we started to search for CVEs related to these versions of the plugins, getting to the [CVE-2021-34646](https://www.cvedetails.com/cve/CVE-2021-34646), which has an [exploit](https://www.exploit-db.com/exploits/50299) that we searched for in the Exploit Database.
...
We saved the output of the program after commenting the proposed line and, after comparing the two using `diff` command, we concluded there are no differences between the outputs. 

<figure align="center">
  <img src="images/week4/diffEnvVars.png" alt="Task2.2"/>
  <figcaption>Figure 6. Zero differences between Environment variables of both processes</figcaption>
</figure>

This means that both processes (parent and child) share the same environment variables, this is, when the parent used `fork()` to create the child process, it creates its own exact duplicate and one of the things that the child process inherits are the environment variables.
...
<figure align="center">
  ...
  <figcaption>Figure 7. Running `myenv.c`</figcaption>
</figure>
...
Looking at the output of the program, we realised that only the `$ANY_NAME` was available to the program.
...
Only the `$ANY_NAME` variable appear, because the `$LD_LIBRARY_PATH` is considered insecure because it might allow unauthorized programs access to system/malicious libraries, for example.

`sudo chmod 4755` uses the first number (4) to Set UID which means you'll run the file as the owner regardless of which user is running it. That's a Set-UID process.

With all this, the superuser has an enabled by default protection system that results in SUID programs to ignore certain environment variables that are blacklisted. `$LD_LIBRARY_PATH` is one of them as we saw.
